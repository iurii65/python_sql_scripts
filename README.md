...
**Входные данные:** Таблица object_analisys из базы данных с полями:
...
<br/><br/><br/>
### Скрипт №6: SPARK_script_FIFO

**Входные данные:** файл list_company.csv со списком компаний, файл c остатками в разрезе компаний на дату, SQL - таблица с приходными накладнымми 

**Назначение:** Распределение номенклатуры остатков по приходным накладным с учетом то, что в компании в качетсве расчета себестоимости примеяется метод FIFO.

**Описание работы:** Первоначально происходит чтение файла со списком компаний, цикл проходит по ИНН и наименованию компаний. Далее происходит чтение CSV файла с остаками с фильтром по ИНН полученный ранне из цикла. С помощью SQL запроса с фильтрацией по ИНН получаем приходные накладные. 
